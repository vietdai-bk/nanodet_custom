...
class PPLCNet(nn.Module):
    ...
    #     return x
    def forward(self, x):
        feature_maps = []  # Danh sách lưu feature maps

        for layer in self.features:
            x = layer(x)
            if isinstance(layer, DepSepConv) and layer.conv[0].stride == (2, 2):
                feature_maps.append(x)
                print(f"Feature map size: {x.shape}") 
        ...
        return feature_maps[-3:]  # Chỉ lấy 3 feature maps cuối
